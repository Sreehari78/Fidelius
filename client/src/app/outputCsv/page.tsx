...
export default function MultiFileCsvViewer() {
  ...
  useEffect(() => {
    const loadCsvFiles = async () => {
      ...
      try {
        const outputFiles: string[] = JSON.parse(decodeURIComponent(filesParam))
        ...
        if (datasets.length > 0) {
          const initialVisibility = datasets[0].headers.reduce((acc, header) => {
            acc[header] = true
            return acc
    ...
    <div className="container mx-auto p-4">
      ...
      {csvDatasets.length === 0 ? (
        <div className="text-center">
          <p className="text-gray-600">No modified files available to display.</p>
        </div>
      ) : (
        <>
          <div className="mb-4">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                const newVisibility = csvDatasets[newIndex].headers.reduce((acc, header) => {
                  acc[header] = true
                  return acc
                }, {} as Record<string, boolean>)
                ...
                {memoizedTable.getRowModel().rows.map((row) => (
                  <tr key={row.id}>
                    {row.getVisibleCells().map((cell) => (
                      <td key={cell.id} className="px-4 py-2 text-sm text-gray-700 border">
